...
API_HEADERS = dict(st.secrets["api"].get(
    "headers", {})) if get_secret("api", "headers") else {}
        ...
        if os.path.exists(filepath):
            ...
            try:
                ...
                if 'nom_AreaTelefonica' in df.columns and 'qtd_Acessos' in df.columns:
                    ...
                    df['num_MesAno'] = df['num_MesAno'].astype(
                    ...
                    dict_share = df_latest.groupby('nom_AreaTelefonica')[
                        'qtd_Acessos'].sum().to_dict()
                    print(
...
def processar_dados(df_raw, filtros_contrato):
    ...
    if isinstance(filtros_contrato, str):
        ...
        df = df[df['Contrato_Padrao'].isin(
            [c.upper() for c in filtros_contrato])].copy()

    df['Abertura_dt'] = pd.to_datetime(
        df['Abertura'], errors='coerce').dt.tz_localize(None)
...
def gerar_cards_mpl(kpis, contrato):
    ...
    def draw(x, y, w, h, t, v, col=C_TEXT):
        ax.add_patch(patches.FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0,rounding_size=3", fc="white", ec=C_BORDER, lw=2))
        ax.text(x+w/2, y+h*0.8, t.upper(), ha='center',
                size=18, color=C_LABEL, weight='bold')
        ax.text(x+w/2, y+h*0.4, str(v), ha='center',
                size=55, color=col, weight='black')
    ax.text(50, 96, "SIGMA OPS", ha='center',
            size=32, weight='black', color='#7c3aed')
    ax.text(50, 92, f"{contrato} • {datetime.now().strftime('%H:%M')}",
            ha='center', size=22, weight='bold', color='#475569')
...
def gerar_lista_mpl_from_view(df_view, col_order, contrato):
    import matplotlib.pyplot as plt
    ITENS_POR_PAGINA = 20
    cols = [c for c in col_order if c in df_view.columns and c not in [
        'horas_float', 'Status SLA']]
    ...
    for i in range(num_paginas):
        ...
        plt.title(titulo, loc='center', pad=40, fontsize=28,
                  weight='black', color='#1e293b')
        tbl = ax.table(cellText=df_chunk.values.tolist(
        ), colLabels=df_chunk.columns, cellLoc='center', loc='center')
...
def gerar_dashboard_gerencial(df_geral, contratos_list):
    ...
    plt.title(f"VISÃO CLUSTER\nConsolidado SigmaOPS • {hora}",
              loc='center', pad=40, fontsize=28, weight='black', color='#1e293b')

    tbl = ax.table(cellText=resumo.values.tolist(),
                   colLabels=resumo.columns, cellLoc='center', loc='center')
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(11)
    tbl.scale(1.2, 3.0)
...
if df_raw is not None:
    ...
    with tab_op:
        ...
        with c_sel:
            if CONTRATO and PERFIL not in ["master", "admin"]:
                ...
                contrato_atual = st.radio(
                    "Selecione o Contrato:", CONTRATOS_VALIDOS, horizontal=True, label_visibility="collapsed")
        with c_ref:
        ...
        # KPIs HTML
        t = len(df_view)
        k = {'total': t, 'sem_tec': len(df_view[df_view['Técnicos'] == 0]), 'critico': len(df_view[df_view['Status SLA'] == 'Crítico']), 'fora': len(df_view[df_view['Status SLA'] == 'Fora do Prazo']), 'no_prazo': len(
            df_view[df_view['Status SLA'] == 'No Prazo']), 'lit': len(df_view[df_view['Area'] == 'Litoral']), 'vale': len(df_view[df_view['Area'] == 'Vale'])}
        ...
        if gv > 0:
            st.markdown(
                f"<div class='alert-box'>🚨 {gv} GRANDE(S) VULTO(S) EM ABERTO</div>", unsafe_allow_html=True)
        ...
        with st.expander("📂 Opções de Exportação"):
            c1, c2 = st.columns(2)
            try:
                c1.download_button("Baixar Resumo", gerar_cards_mpl(
                    k, contrato_atual), f"resumo_{nome_arq}.jpg", "image/jpeg", width='stretch')
            except:
                pass

            cols_export = ['Ocorrência', 'Cabo/Primária', 'AT', 'Afetação', 'Reincidência',
                           'Origem', 'Horas Corridas', 'Status SLA', 'VIP', 'Cond. Alto Valor', 'B2B', 'Técnicos']
            try:
                ...
                if imgs:
                    if len(imgs) == 1:
                        c2.download_button(
                            "Baixar Lista", imgs[0], f"lista_{nome_arq}.jpg", "image/jpeg", width='stretch')
                    else:
                        for idx_img, img_bytes in enumerate(imgs):
                            c2.download_button(
                                f"Baixar Lista (Pág {idx_img+1})", img_bytes, f"lista_{nome_arq}_p{idx_img+1}.jpg", "image/jpeg", width='stretch')
            except:
                pass
        ...
        with c_tab2:
            layout_modo = st.selectbox("📱 Visualização", [
                                       "🖥️ PC (Completa)", "📱 Telemóvel (Resumida)"], label_visibility="collapsed")
        ...
        tabela_html = (
            df_tela.style
            .apply(highlight_rows_tela, axis=1)
            .set_table_attributes('style="width:100%; border-collapse: collapse; font-family: Inter, sans-serif;"')
            .set_table_styles([
                {'selector': 'th', 'props': [
                    ('text-align', 'center'),
            ...
            if sels:
                ...
                with st.expander("Descarregar Imagem"):
                    if st.button("Gerar Dashboard"):
                        try:
                            st.download_button("Download", gerar_dashboard_gerencial(
                                df_cl, sels), f"cluster_{nome_arq}.jpg", "image/jpeg")
                        except:
                            pass
