...
def processar_dados(df_raw, filtros_contrato):
    ...
    df["Abertura_dt"] = pd.to_datetime(df["Abertura"], errors="coerce").dt.tz_localize(
        None
    )
...
def gerar_lista_mpl_from_view(df_view, col_order, contrato):
    ...
    for i in range(num_paginas):
        ...
        plt.title(
            titulo, loc="center", pad=40, fontsize=28, weight="black", color="#1e293b"
        )
        tbl = ax.table(
...
if df_raw is not None:
    ...
    with tab_op:
        ...
        ocultar_final = [
            dict_renomear.get(c, c) for c in cols_ocultar_html if c in df_view.columns
        ]
